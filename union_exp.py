...
def visualize_comparison(G_true, W_est, save_path, title):
    ...
    TP = np.where((G_true > 0) & (W_est > 0))
    FP = np.where((G_true == 0) & (W_est > 0))
    FN = np.where((G_true > 0) & (W_est == 0))
...
if __name__ == '__main__':
    ...
    Y_combined_full = np.hstack(Y_list)
    kfilter = KnockoffFilter(
        ksampler='gaussian',
        fstat='lasso',
    )

    rejections = kfilter.forward(X=X_combined_full, y=Y_combined_full, fdr=0.1, shrinkage="ledoitwolf")
    ...
    for k in range(K):
        ...
        selected = perform_knockoff_filtering(X_list[k], Y_list[k], groups=groups, m=m, q=q_pre)
